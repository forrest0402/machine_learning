...
class TripletNetwork:
    ...
    def network(self, x, reuse=True):
        cnn1 = tf.layers.conv2d(tf.expand_dims(x, -1), filters=256, kernel_size=[3, 256],
                                padding="VALID",
                                activation=tf.nn.tanh, name="cnn1", reuse=reuse)
        bn1 = tf.layers.batch_normalization(cnn1, name="bn1", reuse=reuse,
                                            training=self.training)

        cnn2 = tf.layers.conv2d(bn1, filters=512, kernel_size=[5, 1],
                                padding="VALID",
                                activation=tf.nn.tanh, name="cnn2", reuse=reuse)
        bn2 = tf.layers.batch_normalization(cnn2, name="bn2", reuse=reuse,
                                            training=self.training)

        flattern = tf.layers.flatten(bn2, 'flatten_layer')

        fcl1 = tf.layers.dense(flattern, 256, name="fcl1", activation=tf.nn.tanh, reuse=reuse)
        dfcl1 = tf.layers.dropout(fcl1, training=self.training)

        out = tf.layers.dense(dfcl1, 128, name="output", activation=tf.nn.tanh, reuse=reuse)
        print("{}->{}->{}->{}->{}->{}".format(cnn1.name, bn1.name, cnn2.name, bn2.name,
                                              fcl1.name, out.name))
        return out
