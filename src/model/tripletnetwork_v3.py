...
class TripletNetwork:
    ...
    def network(self, x, reuse=True):
        cnn1 = self.cnn_layer(tf.expand_dims(x, -1), "cnn1", [3, 256, 1, 512], reuse=reuse)
        cnn2 = self.cnn_layer(cnn1, "cnn2", [5, 1, 512, 1024], reuse=reuse)

        fc1 = self.fc_layer(tf.layers.flatten(cnn2, 'flatten_layer'), 256, "fc1", last_layer=False,
                            reuse=reuse)
        out = self.fc_layer(fc1, 128, "out", last_layer=True, reuse=reuse)
        return out

    def fc_layer(self, tensor, n_weight, name, last_layer, reuse):
        ...
        # fc = tf.layers.dense(tensor, n_weight, name=name, activation=tf.nn.tanh, reuse=reuse)
        if not last_layer:
    ...
    def cnn_layer(self, tensor, name, shape, reuse):
        ...
        # bn = self.batch_norm(tanh, shape[-1], phase_train=self.training, scope="bn" + name,reuse=reuse)
        pool = tf.nn.max_pool(tanh, ksize=[1, 2, 1, 1], strides=[1, 1, 1, 1], padding="VALID")
        bn = self.batch_norm(pool, shape[-1], phase_train=self.training, scope="bn" + name,
                             reuse=reuse)
        return bn
    ...
    def batch_norm(self, x, n_out, phase_train, reuse, scope='bn'):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            gamma = tf.get_variable(name='gamma',
                                    initializer=tf.constant(1.0, shape=[n_out], dtype=tf.float32))
